...
const cut = (h, n = 6) => !h ? '' : h.length <= 2 * n + 1 ? h : h.slice(0, n) + '…' + h.slice(-n);
...
const RANK = { queued: 0, proving: 1, submitted: 2, confirmed: 3, failed: 0 };
const LABEL = {
...
const col = (k) => CAT[Math.max(0, ORDER.indexOf(k)) % CAT.length];
...
// ------------------------------------------------------------------ bits
function strip(host, cells) {
    host.innerHTML = cells.map((c) => '<div><div class="v">' + esc(c.v) + '</div><div class="k">' + esc(c.k) + '</div>'
...
function hbars(host, rows, unit) {
    ...
    host.innerHTML = rows.map((r) => '<div class="hr" data-tip="' + esc(r.label + ': ' + r.value + ' ' + unit) + '">'
...
function tips(host) {
    ...
    const t = tip;
    for (const el of Array.from(host.querySelectorAll('[data-tip]'))) {
        el.addEventListener('mouseenter', () => { t.innerHTML = el.dataset.tip; t.style.opacity = '1'; });
...
function areaChart(host, pts) {
    if (pts.length === 0) {
        host.innerHTML = '<div class="empty">Activity appears once the first action is anchored.</div>';
        return;
    }
    ...
    const grid = ticks.map((v) => '<line x1="' + L + '" y1="' + Y(v).toFixed(1) + '" x2="' + (W - R) + '" y2="' + Y(v).toFixed(1)
    ...
    const cx = host.querySelector('#cx');
    const cd = host.querySelector('#cd');
    const tp = host.querySelector('.tip');
    host.querySelector('#ct').addEventListener('mousemove', (ev) => {
        ...
        cd.setAttribute('cx', String(px));
        cd.setAttribute('cy', String(py));
    ...
    host.querySelector('#ct').addEventListener('mouseleave', () => {
        cx.setAttribute('opacity', '0');
        cd.setAttribute('opacity', '0');
        tp.style.opacity = '0';
    });
}
function timelineStrip(host, evs) {
    if (evs.length === 0) {
        host.innerHTML = '<div class="empty">The timeline fills as the match is anchored.</div>';
        return;
    }
    const blocks = evs.map((e) => '<i style="background:' + col(e.kind) + (e.status === 'confirmed' ? '' : ';opacity:.35')
...
// ------------------------------------------------------------------ links
const acct = (a) => (data?.explorerBase ? data.explorerBase + '/account/' + a : '');
function txCell(id) {
    if (!id)
        return '<span class="m4">pending</span>';
...
const pill = (s) => '<span class="chip ' + (s === 'confirmed' ? 'good' : s === 'failed' ? 'bad' : 'warn')
    + '"><span class="d"></span>' + esc(s) + '</span>';
// ------------------------------------------------------------------ render
function evList() { return [...events.values()].sort((a, b) => a.seqNo - b.seqNo); }
function renderOverview() {
    ...
    const c = new Map();
    for (const e of evs)
        c.set(e.kind, (c.get(e.kind) ?? 0) + 1);
    hbars($('kinds-chart'), ORDER.filter((k) => c.get(k))
        .map((k) => ({ label: LABEL[k] ?? k, value: c.get(k), color: col(k) })), 'anchored');
    let cum = 0;
    ...
    $('privacy-budget').innerHTML =
        '<div class="kv"><span class="k">Kills anchored, killer seat hidden</span><span class="num">' + kills + '</span></div>'
            ...
            + (anon ? '0%' : 'n/a') + '</span></div>';
    const h = health;
    const kv = (k, v) => '<div class="kv"><span class="k">' + esc(k) + '</span><span>' + v + '</span></div>';
...
function renderDisclosure() {
    ...
    const li = (k, v, sealed = false) => '<li><span>' + esc(k) + '</span><span class="' + (sealed ? 'seal' : 'val') + '">' + v + '</span></li>';
...
function renderVerification() {
    ...
    if (revs.length === 0) {
        ...
        walk.innerHTML = '<b>Seat</b><b>Commitment inputs</b><b>Recomputed</b><b>On chain</b><b>Check</b>'
            + revs.map((r) => '<span class="seat">' + r.seat + '</span>'
    ...
    if (revs.length === 0) {
        body.innerHTML = '<tr><td colspan="7" class="empty">Roles stay sealed until the match ends.</td></tr>';
        return;
    }
...
function renderSeats() {
    ...
    if (!d || d.commitments.length === 0) {
        body.innerHTML = '<tr><td colspan="4" class="empty">Seats are assigned when the match is dealt.</td></tr>';
        return;
    }
...
function renderContract() {
    const d = data;
    const row = (k, v) => '<tr><td style="width:190px" class="m3">' + esc(k) + '</td><td>' + v + '</td></tr>';
...
function renderChrome() {
    ...
    const btn = $('explorer-btn');
    if (testnet && d?.contractAddress) {
        btn.href = acct(d.contractAddress);
...
async function pullData() {
    try {
        const r = await fetch('/audit/data');
        if (!r.ok)
            return;
        data = (await r.json());
...
function connect() {
    ...
    es.onmessage = (m) => {
        try {
            ...
            renderTransactions();
            renderDisclosure();
            if (e.kind === 'deal' || e.kind === 'winner' || e.kind === 'reveal')
                void pullData();
        }
        catch { /* ignore */ }
    };
